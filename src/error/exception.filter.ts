...
export class TypeOrmExceptionFilter implements ExceptionFilter {
  private readonly logger = new Logger('DatabaseException');
    ...
    if (exception instanceof EntityMetadataNotFoundError || exception instanceof CustomRepositoryNotFoundError) {
      ...
      if (code === PostgresErrorCode.UNIQUE_VIOLATION) {
        ...
        if (parsed) {
          ...
          defaultError.fields = [{ field: parsed.field, message: 'Already exists' }];
        }
      } else if (code === PostgresErrorCode.NOT_NULL_VIOLATION && driverErr.column) {
        defaultError.fields = [{ field: driverErr.column, message: 'Field is required' }];
      }
